...
pub fn render_playback_window(
    ...
    if let Some(ref playback) = player.playback {
        if let Some(item) = &playback.item {
            ...
            let duration = crate::utils::get_playable_item_duration(item).unwrap_or_else(|| {
        ...
        let (text, style) = match m.as_str() {
            ...
            "{liked}" => {
                if let Some(track) = &track {
                    if data.user_data.saved_tracks.contains_key(&track.id.uri()) {
            ...
            "{track_number}" => {
                // we don't store track_number in our internal Track struct yet...
                // wait, let's check Track struct
                continue;
            }
            "{artists}" => {
                if let Some(t) = &track {
